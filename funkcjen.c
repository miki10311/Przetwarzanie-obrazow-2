#include"funkcje.h"
/* tutaj sa funkcje do przetwarzanie obrazka PPM - kolor niebieski */
...
void negatywn (ob *obraz)
{
  ...
  obraz->nieb[i][j]=obraz->szarosci-obraz->nieb[i][j];        /* funkcji. Ta zmiana jest zobrazowana w 4 linijce kodu funkji */ 
}
...
void progowanien (ob *obraz,int prog)
{
 ...
 for (j=0;j<obraz->wymx;j++)
  if(obraz->nieb[i][j]<=obraz->szarosci*prog/100.0)           /* ten if sprawdza, czy dany piksel jest pod czy nad podanym progiem w %     */
  {          
    obraz->nieb[i][j]=0;                                      /* kazda wartosc, ktora jest pod progiem dostaje wartosc 0, a kazda, ktora   */
  }                                                           /* jest nad progiem otrzymuje wartosc rowna maksymalnemu odcieniowi szarosci */
  else
  {
    obraz->nieb[i][j]=obraz->szarosci;
  }
}
...
void pol_prog_czernin (ob *obraz,int progc)
{
 ...
 for (i=0; i<obraz->wymy; i++)
 for (j=0; j<obraz->wymx; j++)
  if (obraz->nieb[i][j]<=obraz->szarosci*progc/100.0)               /* ten if sprawdza, czy dany piksel jest pod czy nad podanym progiem w %     */
  {          
      obraz->nieb[i][j]=0;                                          /* kazda wartosc, ktora jest pod progiem dostaje wartosc 0                   */
  }                                                  
}
...
void pol_prog_bielin (ob *obraz,int progb)
{
 ...
 for (i=0; i<obraz->wymy; i++)
 for (j=0; j<obraz->wymx; j++)
  if (obraz->nieb[i][j]>=obraz->szarosci*progb/100.0)               /* ten if sprawdza, czy dany piksel jest pod czy nad podanym progiem w %     */
  {          
      obraz->nieb[i][j]=obraz->szarosci;                            /* kazda wartosc, ktora jest nad progiem dostaje wartosc maks liczby odcieni */
  }                                                              
}
...
void zmiana_poziomown (ob *obraz,int czern,int biel)
{
  ...
  for (j=0; j<obraz->wymx; j++)
  {
    if (obraz->nieb[i][j]<=cz)                                             
    {                                                        /* wartosci ponizej nowej wartosci czerni otrzymuja wartosc 0 */
      obraz->nieb[i][j]=0;                                                
    }     
    else if (cz<obraz->nieb[i][j] && obraz->nieb[i][j]<bie)  /* wartosci posrednie sa rozciagane na wszystkie poziomy szarosci zawarte w obrazie */
    {
      obraz->nieb[i][j]=(obraz->nieb[i][j]-cz)*(obraz->szarosci/(bie-cz));
    }
    else                                                     /* wartosci powyzej nowej wartosci bieli otrzymaja wartosc */
      obraz->nieb[i][j]=obraz->szarosci;                     /* maksymalna dla danego programu */
  }
}
...
void rozm_poziomen (ob *obraz)
{
  ...
  for (j=1; j<obraz->wymx-1; j++)     /* tablicy (segmentation fault), a 1 piksel doda tylko ramke o grubosci 1 piksela */
      obraz->nieb[i][j]=(obraz->nieb[i-1][j]+obraz->nieb[i][j]+obraz->nieb[i+1][j])/3;
}
...
void rozm_pionowen (ob *obraz)
{
  ...
  for (j=0; j<obraz->wymx-1; j++)     /* tablicy (segmentation fault), a 1 piksel doda tylko ramke o grubosci 1 piksela */
      obraz->nieb[i][j]=(obraz->nieb[i][j-1]+obraz->nieb[i][j]+obraz->nieb[i][j+1])/3;
}
...
void konturowanien (ob *obraz) 
{
  ...
  for (i=0;i<obraz->wymy-1;i++)       /* w tym miejscu '-1' w petli, bo w przeciwnym wypadku przejdziemy poza zakres    */
  for (j=0;j<obraz->wymx-1;j++)       /* tablicy (segmentation fault), a 1 piksel doda tylko ramke o grubosci 1 piksela */
      obraz->nieb[i][j]=abs(obraz->nieb[i+1][j]-obraz->nieb[i][j])+abs(obraz->nieb[i][j+1]-obraz->nieb[i][j]);
...
{
  int i,j;
  double max=obraz->nieb[0][0];        /* szukamy wartosci maksymalnej */

  for (i=0; i<obraz->wymy; i++)
  for (j=0; j<obraz->wymx; j++)
  {
      if (max<obraz->nieb[i][j])
          max=obraz->nieb[i][j];
  }
  ...
  {                                                   
    ...
    obraz->nieb[i][j]=x*max;
  }
} 


/* Rozciaganie hitogramu */
void histogramn (ob *obraz)                                                /* moze byc nie zawsze widoczne, bo ta operacja ma sens gdy nie w pelni jest */
{                                                                          /* wykorzystywany zakres odcieni szarosci obrazka */
  int i,j;
  int min=obraz->nieb[0][0],max=obraz->nieb[0][0];                         /* inicjuje w ten sposob, aby potem porownywac z kazdym innym elementem */
                                                                           /* w ten sposob unikam bledow wartosci skrajnych */
  for (i=0; i<obraz->wymy; i++)
  for (j=0; j<obraz->wymx; j++)
  {
      if (min>obraz->nieb[i][j])                                           /* szukanie wartosci minimalnej i maksymalnej odcieni szarosci w calym obrazku */
          min=obraz->nieb[i][j];
  ...
  for (i=0; i<obraz->wymy; i++)
  for (j=0; j<obraz->wymx; j++)
      obraz->nieb[i][j]=(obraz->nieb[i][j]-min)*(obraz->szarosci/(max-min));    /* rozciaganie wartosci wszystkich pikseli (o ile ma to sens) do wartosci bardziej */
